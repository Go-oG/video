...
private fun loadParameters(cls: Class<*>): List<Parameter> {
    ...
    for (pp in cls.javaClass.declaredFields) {
        ...
        if (type == fdcls) {
            ...
            val rp = Parameter(name, minV, maxV, step, value.getCurrent(), true)
            ...
            val rp = Parameter(name, minV.toFloat(), maxV.toFloat(), step.toFloat(), value.getCurrent().toFloat(), false)
            parameterList.add(rp)
        }
    }
